...
| --- | --- | --- | --- |
| `dim_customer` | Table | One active pseudonymous customer | `customer_key` |
| `dim_service` | Table | One accepted service validity period | `service_version_key` |
| `dim_date` | Table | One calendar day in the configured inclusive range | `date_key` |

### `dim_customer`

`dim_customer` is an exact Type-1 current projection of `int_customer_protected`. It contains ten
customer keys plus `customer_segment`, `is_active`, and `source_updated_at`.
...
not manage current-row flags, period gaps, or period overlap.
...
Deleted customer keys are absent from the four customer-bearing Layer3 relations—`dim_customer`,
`dim_service`, `fct_customer_event`, and `fct_invoice`—and from all four case views. `dim_date` has
no customer key.

The case-view deletion dbt test references an ephemeral deletion model that reads Layer1 and calls
